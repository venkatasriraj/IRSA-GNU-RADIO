...
import pkt_rcv_strip_epy_block_1 as epy_block_1  # embedded python block
import pkt_rcv_strip_epy_block_2 as epy_block_2  # embedded python block
import sip
import threading
...
class pkt_rcv_strip(gr.top_block, Qt.QWidget):
    ...
    def __init__(self, Infile='log.txt', iq='iq_samplesRx.dat', timestamp='timestampsRx.txt'):
        ...
        self.settings = Qt.QSettings("gnuradio/flowgraphs", "pkt_rcv_strip")
...
def argument_parser():
    ...
    parser.add_argument(
        "--iq", dest="iq", type=str, default='iq_samplesRx.dat',
        help="Set filename [default=%(default)r]")
    parser.add_argument(
...
def main(top_block_cls=pkt_rcv_strip, options=None):
    ...
    tb = top_block_cls(Infile=options.Infile, iq=options.iq, timestamp=options.timestamp)
